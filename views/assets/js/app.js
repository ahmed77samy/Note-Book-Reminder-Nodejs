...
(function (jQuery) {
  ...
  $('#form-note-submit').on('click', (e) => {
    $('#form-note-body-submit').val($('#quill-toolbar .ql-editor').html());
  });

  $('.btn-update-note').on('click', (e) => {
    $('.form-note-body-update').val($(e.target).parent().parent().parent().find('.ql-editor').html());
    $.ajax({
      url: e.target.getAttribute('data-req'),
      type: 'PATCH',
      data: {
        noteBody: $(e.target).parent().parent().parent().find('.form-note-body-update').val(),
      },
    }).done(() => {
      location.reload();
    });
  ...
  $('.note-detail .iq-icon-box-2').each(function (i, el) {
    $(this).click(function (e) {
      if ($(this).hasClass('active')) {
        console.log('is active');
      } else {
        $.ajax({
          url: $(this).attr('data-req'),
          type: 'PATCH',
          data: {
            privacy: $(this).attr('data-value'),
          },
        }).done(() => {
          location.reload();
        });
